...
**Sky2 Chest Tracker** 是一个 Windows 原生 Mod，在游戏地图上显示宝箱位置，并区分本周目与存档继承记录，辅助查找遗漏宝箱。

当前版本 **0.4.0** · [下载安装包](https://github.com/blockshy/sky2-chest-tracker/releases/latest) · [构建指南](https://github.com/blockshy/sky2-chest-tracker/blob/main/docs/BUILDING.md) · [更新记录](https://github.com/blockshy/sky2-chest-tracker/blob/main/CHANGELOG.md)

## 功能

- 使用游戏原生地图图标显示对应区域的宝箱，包括尚未发现的宝箱。
- 切换“本周目／继承记录”显示模式，闭合箱表示当前模式下未开，开启箱表示已开。
- 同时显示本周目与继承记录的全局计数和当前地区计数。
...
- 提供默认关闭的地图全显与未到访传送点辅助；传送点按当前地区的原生菜单规则临时显示，支持地图内即时切换。具体范围见下方操作说明。
...
1. 从 [Releases](https://github.com/blockshy/sky2-chest-tracker/releases) 下载 `Sky2ChestTracker-0.4.0.zip` 并完整解压。GitHub 自动生成的 **Source code** 压缩包只有源码，不能直接安装。
2. 保存进度并正常退出游戏。
...
1. 下载并完整解压 `Sky2ChestTracker-0.4.0.zip`，保存进度并退出游戏。
...
5. 从 Steam 启动游戏，确认面板显示“宝箱追踪 · 0.4.0”。
...
| 键盘 | Xbox 手柄 | 功能 |
| --- | --- | --- |
| F6 | View + X | 切换模式：本周目／继承记录 |
| F7 | View + B | 显示/隐藏面板，地图标记继续生效 |
| F8 | View + A | 打开／收起各地图收集清单 |
| F9 | View + RS | 暂停／恢复宝箱地图标记，暂停时宝箱使用原版显示 |
| F10 | View + Y | 清单切换全部／仅看遗漏 |
| PgUp | View + LB | 清单上一页 |
| PgDn | View + RB | 清单下一页 |

各地图收集清单的页码位于顶部“已完成 … 张地图／剩余未开 …”统计行右侧，与内容区右边缘对齐。
清单宽度综合完整地图路径、计数列、顶部统计与快捷键所需空间确定，减少“大地图 / 地点”列的多余留白；窄窗口中的长路径仍可换行显示。
底部快捷键采用三列两行：第一行依次为“切换模式”“全部/遗漏”“收起清单”；第二行显示“上一页”“下一页”。
每个按键与对应动作相邻，按键使用与主面板一致的高亮颜色。手柄提示逐项显示完整的 `View + X`、`View + Y`、`View + A`、`View + LB`、`View + RB`，不省略共用的 View 前缀。
主面板直接显示按键与动作提示；宽度按实际文字和当前输入方式测量，同时完整显示两组地区计数、组合键与探索状态。

探索辅助提供两组独立开关：

| 键盘 | Xbox 手柄 | 功能 |
| --- | --- | --- |
| Ctrl + F6 | View + 十字键 ↑ | 地图全显开／关 |
| Ctrl + F8 | View + 十字键 ↓ | 未到访传送点开／关 |

面板将名称与状态直接连接，例如“地图全显：已关闭”；对应组合键单独高亮，与上方宝箱快捷键的右侧按键列对齐。
两个开关均默认关闭，重启后关闭，F9 不影响它们。传送开关在区域地图、全地点列表和城镇子列表的浏览阶段原地生效，
无需关闭再打开地图；确认窗口或转场期间会等待返回浏览阶段后应用。刷新保留地图镜头和缩放；
若选中的临时目的地消失，原生地图会重新选取有效地点，列表滚动位置可能重置。
若切换后当前子列表或整个传送清单合法为空，地图会正常关闭；功能仍保持可用。

“未到访传送点”自动枚举当前地区的原生传送记录：目的地已被游戏登记、最终未被禁用，且所属分组未被禁用时，
允许它在尚未到访的情况下临时显示，必要时同时显示其地点分组。目的地及分组仍受游戏的原生条件、内部点和特殊标识过滤，
没有固定的点位数量，也不要求先到访某个城镇或道路节点。确认窗口和传送前后脚本继续由游戏执行。
这采用的是**原生菜单许可**，不等于完整剧情可达判断；可能越过入口剧情、物理障碍或迷宫层级。测试步骤见 [探索辅助测试指南](https://github.com/blockshy/sky2-chest-tracker/blob/main/docs/EXPLORATION_TESTING.md)。
全显只影响原生当前加载／选中地图的可探索分块，不补充剧情尚未提供的地图目录。
Mod 不批量写探索和到访记录；实际行走或传送仍按游戏机制记录真实进度。
...
面板顶部以“显示模式”标明当前选择，每次启动默认为本周目。使用“切换模式”只改变地图图标、剩余未开数量和遗漏筛选，两组统计始终独立显示。
...
- **继承视图与本周目图标不同：**这是预期行为；以前周目已开的宝箱，本周目仍可能未开。
- **手柄组合不响应：**先切回游戏并松开所有控制，再先按 View 后按功能键；确认使用当前版本，RS 需要按下。
- **宝箱图标恢复成原版：**查看面板是否显示“宝箱标记已暂停（原版显示）”，使用 F9 或 View + RS 恢复宝箱标记。
...
地图全显、统一未到访传送点、地图内即时刷新及对应组合键已获得用户本机测试正常反馈；发布前的整体验证也已确认正常。
这些反馈不表示逐个地点、全章节组合均已验证。探索适用范围和回归步骤见 [探索辅助测试指南](https://github.com/blockshy/sky2-chest-tracker/blob/main/docs/EXPLORATION_TESTING.md)。
